...
| `y_axis_label(ax, text, *, unit=None)`                              | Horizontal title heading the y tick-label column — flush with the column's outer edge (over the "100%", not the spine) and seated above its top label; `unit=` renders below in muted colour. Call BEFORE `finalize` — it reserves a band under the descriptor and re-anchors the label to the final axes top and column, clear of in-axes text poking past the axes edge and (for `side="left"`) above the `panel_label` band. A `top_legend` that clears the block horizontally shares its strip (legend left, label right) instead of stacking a second band. |
